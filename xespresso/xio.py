...
def build_atomic_species_str(atoms, input_parameters, pseudopotentials):
    ...
    for species in species_info:
        ...
        atomic_species_str.append(
            "{species} {mass} {pseudo}\n".format(
                species=species, mass=species_info[species]["mass"], pseudo=pseudo
            )
        )
    atomic_species_str.append("\n")
...
def write_neb_in(
    ...
    **kwargs
    ...
    if climbing_images:
        ...
        climbing_images_str = "   "
        for x in climbing_images:
            climbing_images_str += " %s" % x
    ...
    for i in range(len(images)):
        ...
        if i == 0:
            ...
            atomic_positions_str.append("\nINTERMEDIATE_IMAGE\n")
        atomic_positions_str.extend(
            build_atomic_positions_str(atoms, crystal_coordinates)
        )
    ...
    with open(pwo) as f:
        ...
        for idx, line in enumerate(lines):
            if "number of atoms/cell" in line:
                ...
                atomic_species = [
                    at_line.split()[1] for at_line in lines[idx + 1 : idx + 1 + nat]
                ]
                break
...
def read_espresso_input(fileobj, neb=False):
    ...
    pseudopotentials = {}
    trimmed_lines = (line for line in card_lines if line.strip() and not line[0] == "#")
...
def get_constraint(constraint_idx):
    ...
    mask = [[(ic + 1) % 2 for ic in c] for c in constraint_idx if np.all(c is not None)]
