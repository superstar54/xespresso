...
class OER_base(Base):
    def __init__(
        ...
        self.mols = {
            ...
            "OOH": Atoms(
                "OOH", positions=[[0, 0, 0], [0.8, 0.9, 0.0], [1.4, 1.6, 0.45]]
            ),
        }
        """
...
class OER_bulk(OER_base):
    ...
    def build_children(self, surfaces=None, nlayer=None, fix=None, tol=None):
        ...
        for index, ters in self.indexs.items():
            ...
            for ter, surf in terminations.items():
                ...
                surf = OER_pourbaix(
                    surf,
                    label=os.path.join(self.label, prefix),
                    prefix=prefix,
                    calculator=self.calculator,
                    view=self.view,
                    molecule_energies=self.molecule_energies,
                    **self.pourbaix_input,
    ...
    def get_terminations(self, slab, layers):
        ...
        for i in range(nlayer - 2, 0, -1):
            ...
            print(formula)
            inds = [
                j for j in range(natoms) if layers[j] in range(i, i - self.nlayer, -1)
            ]
            atoms = slab[inds]
            atoms.wrap()
            if not terminations:
...
class OER_site(OER_base):
    def __init__(
        ...
        if site_type.upper() == "ONTOP":
            ...
            self.site_position = (
                atoms[site[0]].position
                + atoms[site[1]].position
                + atoms[site[2]].position
            ) / 2.0 + np.array([0, 0, height])
    ...
    def build_oer_adsorbate(
        ...
        self.log("-" * 60)
        self.log(
            "Adsoption site: index ({0})  symbol({1})".format(
    ...
    def get_free_energy(self, results=None, G_O=None, G_OH=None, G_OOH=None, E_H=None):
        ...
        dG_OH = (
            self.results["%s_%s" % (label, "OH")]["energy"]
            - E0
            - G_OH
            + self.zpes["%s_%s" % (label, "OH")]
        )
        dG_O = (
            self.results["%s_%s" % (label, "O")]["energy"]
            - E0
            - G_O
            + self.zpes["%s_%s" % (label, "O")]
        )
        dG_OOH = (
            self.results["%s_%s" % (label, "OOH")]["energy"]
            - E0
            - G_OOH
            + self.zpes["%s_%s" % (label, "OOH")]
        )
        dG_O2 = 4.92
        ...
        self.over_potential = (
            max(
                [
                    self.free_energies[1] - self.free_energies[0],
                    self.free_energies[2] - self.free_energies[1],
                    self.free_energies[3] - self.free_energies[2],
                ]
            )
            - 1.23
        )
    ...
    def write_image(
        ...
        for job in self.results:
            ...
            write(
                os.path.join(self.label, "%s-side.png" % job),
                atoms * (2, 2, 1),
                rotation="-90x",
            )
            write(
                os.path.join(self.label, "%s.png" % job),
                atoms * (2, 2, 1),
                rotation="10z,-80x",
            )
