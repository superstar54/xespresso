...
class HpXEspresso(Espresso):
    ...
    def read_results_fromdirectory(self, directory):
        ...
        for pwo in pwos:
            ...
            try:
                ...
                atomic_species = get_atomic_species(pwo)
                if atomic_species:
                    output.arrays["species"] = atomic_species
    ...
    def is_insulator(self, fermi=None):
        ...
        if fermi == None:
            ...
            if fermi == None:
                raise Exception(
                    "I cannot get the Fermi energy from the pw.pwo file"
        ...
        elif any(i[0] == fermi for i in max_mins) and any(
            i[1] == fermi for i in max_mins
        ):
            return False, 0.0, fermi, fermi
        # insulating case
        else:
    ...
    def get_magnetizations(self, getall=False):
        ...
        if getall:
            ...
            return [mags[k][-1] if mags[k] else 0.0 for k in sorted(mags.keys())]
    ...
    def read_Hubbard_parameters(self, iteration):
        ...
        if (
            "lda_plus_u_kind" not in self.parameters
            or self.parameters["lda_plus_u_kind"] != 1
        ):
            start = "site n."
            hp_U = dict()
            ...
            with open(filename, "r") as fobj:
                ...
                end = [n for n, line in enumerate(lines) if "=--------------" in line][
                    1
                ] - 1
    ...
    def run_relax(self):
        ...
        if (
            self.relax_parameters != None
        ):  # This works if the execution of the script is not interrupted
        ...
        if self.iteration > 1:
            if (
                "lda_plus_u_kind" in self.parameters["input_data"]
                and self.parameters["input_data"]["lda_plus_u_kind"] == 2
            ):
                old_parametersin = os.path.join(
                    self.directory_ref, "hp_" + str(self.iteration - 1), "parameters.in"
                )
                shutil.copy(
                    old_parametersin, self.directory, symlinks=False, ignore=None
                )
    ...
    def run_scf(self):
        ...
        if self.sc_type == "relax":
            ...
            self.save_directory_old = os.path.join(
                self.relax_directory, "%s.save" % self.prefix
            )
            self.directory = os.path.join(
            ...
            if self.iteration == 1:
                if self.insulator == None:
                    sys.exit(
                        "You need to provide a value (True/False) for  'insulator'"
                    )
                else:
                ...
                previous_run = os.path.join(
                    self.directory_ref, "scf_" + str(self.iteration - 1)
                )
                self.read_results_fromdirectory(directory=previous_run)
                insulator = self.is_insulator()[0]
                magnetic = self.get_number_of_spins()
        ...
        if insulator == True and magnetic == 1:
            ...
            if (
                ...
                in self.parameters["input_data"]["input_ntyp"].keys()
            ):
                del self.parameters["input_data"]["input_ntyp"][
                    "starting_magnetization"
                ]
            ...
            if self.sc_type == "relax":
                sys.exit(
                    "Case not implemented: is an insulator? {} and is spin polarized? {}".format(
                        self.is_insulator()[0], self.get_spin_polarized()
                    )
                )
            else:
                sys.exit(
                    "Case not implemented: is an insulator? {} and is spin polarized? {}".format(
                        self.insulator, self.magnetic
                    )
                )
    ...
    def run_scf2(self):
        ...
        if self.get_number_of_spins() == 2 and self.is_insulator()[0] == True:
            ...
            nbnd = len(self.calc.get_eigenvalues(kpt=0))
            self.parameters["input_data"]["nbnd"] = nbnd
            self.parameters["input_data"]["tot_magnetization"] = abs(
                self.get_magnetizations(True)["total magnetization"][-1]
            )
            ...
            # create working directory
            self.save_directory_old = os.path.join(
                self.scf_directory, "%s.save" % self.prefix
            )
            self.directory = os.path.join(
                self.directory_ref, "scf2_" + str(self.iteration)
            )
            self.save_directory = os.path.join(self.directory, "%s.save" % self.prefix)
            self.label = os.path.join(self.directory, self.prefix)
            self.scf_directory = self.directory
    ...
    def run_hp(self):
        ...
        if self.scf_directory == None:
            if os.path.exists(
                os.path.join(self.directory_ref, "scf2_" + str(self.iteration))
            ):
                self.scf_directory = os.path.join(
                    self.directory_ref, "scf2_" + str(self.iteration)
                )
            else:
                self.scf_directory = os.path.join(
        ...
        if self.parallelization_hp == False:
            ...
            self.directory = os.path.join(
                self.directory_ref, "hp_" + str(self.iteration)
            )
            ...
            self.post(
                package="hp",
                queue=self.queue_hp,
                parallel=self.parallel_hp,
                **self.hp_parameters
            )
        else:
            sys.exit(
                "Parallelization of the hp.x calculation over the atoms or q-points not implemented."
            )
